...
Author: @vsoch
November 12, 2018 (no longer Halloween :( )
...
def extract(dockerfile, uri, output_file, container_diff_input=None):
    '''extract a dataset from a given dockerfile, write to html output file.
       Use the container_diff_input (json extracted on sherlock) to add 
       software requirements. Yes, the person is hardcoded as @vsoch the 
       Dinosaur. This is my example and I can do that.
    '''
    ...
    image.properties['about'] = 'This is a Dockerfile provided by the Dinosaur Dataset collection.'
    image.properties['codeRepository'] = 'https://www.github.com/openschemas/dockerfiles'
    image.properties['runtime'] = 'Docker'

    # container-diff - run on sherlock
    if container_diff_input != None:
        layers = read_json(container_diff_input)
    else:
